...
class TurboLogitsProcessor(nn.Module):
    ...
    def _standard_topk_topp(
        self,
        candidate_scores: torch.Tensor,
        candidate_indices: torch.Tensor,
        scores: torch.Tensor,
        cumulative_cutoff: torch.Tensor | float,
    ) -> torch.Tensor:
        del candidate_scores, candidate_indices
        ...
        sorted_indices_to_remove = cumulative_probs <= cumulative_cutoff
    ...
    def _compact_topk_topp(
        ...
        cumulative_probs = sorted_logits.softmax(dim=-1).cumsum(dim=-1)
        remove_ascending = cumulative_probs[..., -top_k:] <= cumulative_cutoff
    ...
    def forward(
        ...
        if use_compact_topk_topp:
            ...
            cumulative_cutoff = scores.new_tensor(1 - self.top_p)
            scores = torch.cond(
                has_relevant_tie,
                self._standard_topk_topp,
            ...
            scores = self._standard_topk_topp(
                scores,
                scores,
                scores,
                1 - self.top_p,
