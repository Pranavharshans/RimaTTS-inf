        ...
        try:
            for i in tqdm(
                ...
                if compile_decode and compile_mode == "reduce-overhead":
                    ...
                    cache_copies = [torch.empty_like(tensor) for tensor in cache_tensors]
                    torch._foreach_copy_(cache_copies, cache_tensors)
                    for layer_index, layer in enumerate(initialized_layers):
                        layer.keys = cache_copies[2 * layer_index]
                        layer.values = cache_copies[2 * layer_index + 1]
        finally:
